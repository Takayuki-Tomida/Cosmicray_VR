...
public class EffectIntro80km_2 : MonoBehaviour
//フィールド（メンバ変数）の定義ここから↓
...
{   //クラス名　変数名;　でインスタンスを格納する変数を定義
    ...
    public AudioClip se;//AudioClipクラスのインスタンスを格納する変数seの宣言
    ...
    public static void ShowerAdd(float x, float y, float z, float a, float b, float c, string name, float d, float e)
    {
        ...
        g.transform.LookAt(new Vector3(0, 53.67f, 0));//y軸に対して平行になるように一度向きを決める
